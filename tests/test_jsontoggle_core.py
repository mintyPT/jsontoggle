...
def test_get_json_node(demo_json_path: Path, toggles_dir: Path):
    manager = JsonToggleManager(demo_json_path, toggles_dir)
    assert manager._get_json_node(manager.json_data, ["featureFlags", "newDashboard"]) is True
    assert manager._get_json_node(manager.json_data, ["settings", "theme"]) == "dark"
    assert manager._get_json_node(manager.json_data, ["users", "0", "name"]) == "Alice"
    assert manager._get_json_node(manager.json_data, ["nonExistent"]) is None
    assert manager._get_json_node(manager.json_data, ["users", "10"]) is None # Index out of bounds

def test_set_json_node(demo_json_path: Path, toggles_dir: Path):
    manager = JsonToggleManager(demo_json_path, toggles_dir)
    
    # Test setting a dict value
    manager._set_json_node(manager.json_data, ["featureFlags", "darkMode"], True)
    assert manager.json_data["featureFlags"]["darkMode"] is True

    # Test setting a list value
    manager._set_json_node(manager.json_data, ["users", "0", "name"], "Alicia")
    assert manager.json_data["users"][0]["name"] == "Alicia"

    # Test setting a non-existent path (should return False)
    assert manager._set_json_node(manager.json_data, ["newRootKey", "subKey"], "value") is False
...
def test_toggle_node_in(demo_json_path: Path, toggles_dir: Path):
    ...
    manager.toggle_node(path_to_toggle)
    assert manager._get_json_node(manager.json_data, path_to_toggle.split('.')) is True
